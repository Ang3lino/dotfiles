...
if command -v brew &>/dev/null; then
  ...
  sudo apt install -y zsh tmux git neovim fzf zoxide ripgrep fd-find jq build-essential pkg-config libssl-dev
  if ! $MINIMAL; then
  ...
  sudo dnf install -y zsh tmux git neovim fzf zoxide ripgrep fd-find jq gcc gcc-c++ make pkg-config openssl-devel
  if ! $MINIMAL; then
